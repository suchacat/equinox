...
proc getVendorType*: string =
  ...
  if *vndkStr:
    ...
    if haliumVer == 0:
      ...
      raise newException(Defect, "Unsupported NDK version found! If you believe this is a mistake, file a bug report.")
...
proc getImages*: ImagePair =
  ...
  let systemReq = httpGet(config.systemOta & '/' & config.romType & "/waydroid_" & config.arch & '/' & config.systemType & ".json")

  if systemReq.code != 200:
    raise newException(ImageFetchFailure, "Failed to get system OTA channel: " & systemReq.url & " (" & $systemReq.code & ')')
  
  let systemImages =
    fromJson(systemReq.body, ImageResponse)
      .response
  ...
  for vendor in [deviceCodename, getVendorType()]:
    let vendorReq = httpGet(config.vendorOta & "/waydroid_" & config.arch & '/' & vendor & ".json")
    if vendorReq.code == 200:
      info "container/image: found vendor image list for vendor: " & vendor
      
      let vendorImages =
        fromJson(vendorReq.body, VendorImageResponse)
          .response
          .sortedByIt(it.datetime)
          .reversed()
      ...
      info "Latest vendor image: " & latestVendor.romtype & ' ' & latestVendor.version & " (" & latestVendor.url & ')'
      pair.vendor = latestVendor
    else:
      warn "container/image: cannot find vendor image list for vendor: " & vendor
  
  assert(pair.vendor.version == pair.system.version, "Vendor image and system image are desynchronized! (vendor=" & pair.vendor.version & ", system=" & pair.system.version & ')')
...
proc downloadImages*(pair: ImagePair) =
  ...
  if systemReq.code != 200:
    error "container/image: failed to fetch system image: got non-200 response: " & $systemReq.code
    raise newException(ImageDownloadFailed, "System image request got non-200 response: " & $systemReq.code)

  info "container/image: downloaded system image: " & $(systemReq.body.len.float / (1024'f32.pow(3))) & " GB"
  
  info "container/image: downloading vendor image: " & pair.vendor.url
  let vendorReq = httpGet(pair.vendor.url)

  if vendorReq.code != 200:
    error "container/image: failed to fetch vendor image: got non-200 response: " & $systemReq.code
    raise newException(ImageDownloadFailed, "Vendor image request got non-200 response: " & $vendorReq.code)

  info "container/image: downloaded system image: " & $(vendorReq.body.len.float / (1024'f32.pow(3))) & " GB"
  
  info "container/image: writing system image"
  writeFile(
    config.imagesPath / "system.img",
    systemReq.body
  )

  info "container/image: writing vendor image"
  writeFile(
    config.imagesPath / "vendor.img",
    vendorReq.body
  )
