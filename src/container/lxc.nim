...
type
  BinaryNotFound* = object of Defect
...
proc addNodeEntry*(nodes: var seq[string], src: string, dest: Option[string], mntType, options: string, check: bool): bool {.discardable.} =
...
proc generateNodesLxcConfig*: seq[string] =
  ...
  proc entry(src: string, dest: Option[string] = none(string), mntType: string = "none", options: string = "bind,create=file,optional 0 0", check: bool = true): bool {.discardable.} =
  ...
  entry "none", some("dev/pts"), "devpts", "defaults,mode=644,ptmxmode=666,create=dir 0 0", check = false
  entry "/dev/uhid"
...
proc setLxcConfig*() =
  ...
  var configs = @[CONFIG_BASE.multiReplace(
    substituteTable
  )]
  if lxcMajor <= 2:
    configs &= CONFIG_1
...
proc generateSessionLxcConfig* =
  ...
  var nodes: seq[string]
  proc entry(src: string, dest: Option[string] = none(string), mntType: string = "none", options = "rbind,create=file 0 0"): bool {.discardable.} =
    for x in src:
      if x in { '\n', '\r' }:
  ...
  let
    waylandContainerSocket = absolutePath(config.containerXdgRuntimeDir / config.containerWaylandDisplay)
    waylandHostSocket = absolutePath(config.containerXdgRuntimeDir / config.containerWaylandDisplay)
  
  if not entry(waylandHostSocket, waylandContainerSocket[1 ..< waylandContainerSocket.len].some):
    fatal "equinox: failed to bind Wayland socket!"
    raise newException(OSError, "Cannot bind Wayland socket.\nContainer = " & waylandContainerSocket & "\nHost = " & waylandHostSocket)
