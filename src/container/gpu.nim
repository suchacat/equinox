...
proc getKernelDriver*(device: string): string =
  readFile("/sys/class/drm/" & device & "/device/uevent")
    .split("DRIVER=")[1]
    .split('\n')[0]
...
proc getCardFromRenderNode*(device: string): string =
  ...
  if matches.len < 1:
    ...
    raise newException(InvalidRenderDevice, "found 0 matches for the card that owns this node! (" & device & ')')
  
  if matches.len > 1:
    warn "container/gpu: multiple DRM devices seem to own a single render node: " & device
    warn "container/gpu: this is weird; will it lead to a crash? :P"

  "/dev" / "dri" / matches[0].splitPath().tail

proc getDriNode*: Option[DRINode] =
  for node in glob("/dev/dri/renderD*").walkGlob:
...
proc getVulkanDriver*(device: string): string =
  ...
  if kernelDriver notin table:
    ...
    raise newException(NoVulkanSupport, "Device \"" & device & "\" does not support Vulkan!")
